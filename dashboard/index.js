...
module.exports = function(client) {
	...
	app.get('/soutien', (req, res) => {
		render(res, req, 'soutenir.ejs');
	});
		...
		if (resp.success) {
			...
			if (ip.length) {
				render(res, req, 'message.ejs', {
					message: `Échec de la vérification, vous avez été ${
						(db.get(`punishment_${req.params.guildID}`) || 'kick') === 'kick'
							? 'kicked'
							: 'banned'
					} pour vérifier avec un autre compte Discord!`
				});
				(db.get(`punishment_${req.params.guildID}`) || 'kick') === 'kick'
				...
				let embed = new Discord.MesssageEmbed()
					...
					.addField('Raison', 'Vérifié avec un autre compte')
					.setColor(`#00FF00`);
			...
			render(res, req, 'message.ejs', {
				message: `Vous avez vérifié votre compte avec succès, nous espérons que vous passerez un agréable séjour à ${
					guild.name
				}!`
			});
			...
			render(res, req, 'message.ejs', {
				message: 'Échec de la vérification, rejoignez le serveur et réessayez plus tard'
			});
			member.kick();
		}
	});
	...
	app.post('/api/prefix', (req, res) => {
		...
		if (!prefix)
			return res.json({
				success: false,
				alert: { title: 'Oops!', message: 'Aucun préfixe fourni', type: 'error' }
			});
		if (prefix.length > 4)
			return res.json({
				success: false,
				alert: {
					title: 'Oops!',
					message: 'Le préfixe ne peut avoir que 4 lettres maximum',
					type: 'error'
				}
			});
		...
		return res.json({
			...
			alert: {
				title: 'Success!',
				message: 'Le préfixe a été mis à jour',
				type: 'success'
			}
		});
	});
