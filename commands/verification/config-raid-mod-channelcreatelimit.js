...
module.exports = {
	name: 'config-channelcreatelimit',
	run: async (client, message, args, db) => {
		if (!message.channel.permissionsFor(message.author).has('MANAGE_GUILD'))
			return message.channel.send(":x: | **Vous n'êtes pas autorisé à utiliser cette commande!**");
...
db.set(`channelcreate_${message.guild.id}`, Number(args[0]))
return message.channel.send("**La limite de création de channel a été définie sur " + Number(args[0]) + "**")
